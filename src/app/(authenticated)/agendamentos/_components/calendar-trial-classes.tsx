    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <button
            onClick={goToToday}
            className="px-2 md:px-3 py-1 text-xs md:text-sm font-medium text-purple-600 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors"
          >
            Hoje
          </button>
        </div>
      ...
      <div className="grid grid-cols-7 border-b border-gray-200">
        {dayNames.map((dayName, index) => {
          ...
          return (
            <div
              key={index}
              className={`p-2 md:p-3 text-center text-xs md:text-sm font-semibold text-gray-600 bg-gray-50 ${
                isSunday ? "bg-gray-100 text-red-500" : ""
              }`}
            >
              {dayName}
            </div>
